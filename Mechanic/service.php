...
<?php 
$id = $_SESSION['id'];
$str = "select * from mechanic_master where u_id = $id";
...
<?php else: ?>
	<main class="app-content">
      ...
      <?php $str1 = "select * from mechanic_allocation_master where m_id = $mid";
...
<div class="row d-print-none mt-2">
                 <div class="col-12 text-right"><a class="btn btn-primary" href="javascript:window.print();" target="_blank"><i class="fa fa-print"></i> Print User Information</a> <a class="btn btn-primary" href="mechanicinvoice.php?reqid=<?= $alluid ?>&mid=<?= $mid ?>" target="_blank"><i class="fa fa-file-text-o"></i> View Invoice</a> <a class="btn btn-primary" href="serviceend.php?allid=<?= $allid ?>" target="_blank" onclick="return confirm('Are you sure you want to end your service?')"><i class="fa fa-stop-circle-o"></i> End Service</a></div>
              </div>
          </div>
        </div>
