...
if (isset($btn)) {
  ...
  if ($v->usernamevalidation($username)) {
    ...
    $_SESSION['username'] = "Only Characters and max 10 Characters...";
  }
  ...
  if ($v->onlycharwithnoandspaces($address) == true) {
    $counter++;
  }
  else {
  ...
  if ($counter == 14) {
    ...
    $q1 = mysqli_query($conn, $str);
        ...
        if ($q1 = 1) {
          ...
          $uid    = $result['u_id'];
          $str1   = "insert into hotel_master (h_address,u_id,license) values('$address','$uid','$file_namea')";
          $q2     = mysqli_query($conn, $str1);
          if ($q2 = 1) {
            header("location:registrationsuccess.php?type=Hotel");
          }
        else {
          echo 'tra error';
        }
      ...
      header("location:hotelregistration.php");
    }
  } else {
    header("location:hotelregistration.php");
  }
