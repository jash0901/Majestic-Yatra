   ...
   if(isset($_FILES['idproof'])){
    ...
    if ($counter == 14) {
        ...
        if ($q1 = 1) {
            ...
            $uid    = $result['u_id'];
            $str1   = "insert into vehicle_rental_provider_master (u_id,id_proof,addr,verified) values('$uid','$file_namea','$address',$all)";
            $q2     = mysqli_query($conn, $str1);
            if ($q2 = 1) {
               header("location:registrationsuccess.php?type=Vehicle+Rental+Provider");
            } else {
                echo 'tra error';
            }
        } else {
        ...
        header("location:VehicleRentalProviderregistration.php");
    }
} else {
    header("location:VehicleRentalProviderregistration.php");
}
